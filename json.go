...
func ReadRecord(fileName string, s *[]Record) error {
	if !fileExists(fileName) {
		*s = []Record{}
		return nil
	}
...
func WriteRecord(fileName string, s *[]Record) error {
	f, err := os.Create(fileName)
	if err != nil {
		fmt.Printf("Error occured with creating a file in os.Create()\n")
		return err
...
func AppendRecord(fileName string, s *[]Record) error {
	records := []Record{}
	err := ReadRecord(fileName, &records)
	if err != nil {
		fmt.Printf("Error occured with reding json in ReadJson()\n")
		return err
	}
	records = append(records, *s...)
	err = WriteRecord(fileName, &records)
	if err != nil {
		fmt.Printf("Error occured with writing json in WriteJson()\n")
		return err
	}
